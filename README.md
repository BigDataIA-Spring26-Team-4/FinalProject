...
**YouTube Link** - (https://youtu.be/h98MkpLKNP4)
...
- **Big Data Engineering:** Ingest, process, and store 16+ GB of multi-source maritime and geopolitical data in Snowflake using Airflow-orchestrated batch and near-real-time pipelines with a medallion architecture (Bronze/Silver/Gold).

- **Significant LLM Use:** Deploy a LangGraph multi-agent system with 5 specialist agents (vessel tracker, news analyst, conflict monitor, sanctions checker, weather analyst) using Claude and OpenAI models, connected via MCP tool protocol.
...
- **Data Sources (7 external sources + 1 application-defined reference dataset)**
  ...
  - [GDELT 2.0](https://www.gdeltproject.org) — Global news event database (Events), updates every 15 min
...
- **Cloud Infrastructure**
  - AWS EC2 hosting Docker containers: Airflow, FastAPI, Streamlit, Redis, AIS consumer
...
**Total Expected Volume: ~16 GB** (AIS streaming: 11 GB + GDELT Events: 4.5 GB + remaining sources: ~0.5 GB)
...
| **Caching** | Redis (Docker) | In-memory cache for Snowflake Gold query results and MCP tool responses. Reduces Snowflake compute costs and agent response latency. Chosen over Memcached (less feature-rich, no persistence). |
...
| **Containerization** | Docker + Docker Compose | Multi-service orchestration (Airflow, FastAPI, Redis, Streamlit, AIS consumer). Consistent dev/prod environments. |
...
- **GDELT DAG** (runs every 15 minutes): Queries BigQuery for maritime-relevant events using CAMEO event codes (military actions, sanctions, blockades, naval activity). Downloads Events 2.0 rows into Snowflake Bronze for structured storage.
- **ACLED DAG** (runs weekly, Tuesdays): Fetches conflict events via REST API, filters for coastal/port-adjacent events within 50km of known ports. Structured fields go to Snowflake Bronze.
- **OFAC DAG** (runs daily): Downloads OpenSanctions OFAC consolidated dataset (JSON), parses for maritime-related entities (vessels, shipping companies). Structured entity records go to Snowflake Bronze.
...
The intelligence engine uses a **LangGraph supervisor pattern** with 5 specialist agents, connected to Snowflake via an **MCP server** with a **Redis caching layer** in between:
...
| **News Analyst** | Claude Sonnet | Analyzes GDELT events for maritime impact, assesses escalation | `get_chokepoint_risk` |
...
- **6 Tools**:
  ...
  - `screen_vessel_sanctions` — Checks a vessel MMSI/IMO against OFAC sanctions (queries Snowflake)
...
- **Caching**: All tool responses pass through Redis before hitting Snowflake. Cache hits bypass the data store entirely, reducing both latency and compute cost.
...
2. News Analyst: calls `get_chokepoint_risk("hormuz")` for current aggregate score, reviews recent GDELT Events data → produces risk narrative with citations
...
#### Software Testing
- **Unit Tests** (pytest): ETL transformation functions, Pydantic schema validation, sanctions fuzzy matching, MovingPandas trajectory processing, Redis cache operations
...
#### POC 3: Sanctions Fuzzy Matching
- Download OFAC dataset from OpenSanctions, parse JSON, extract vessel entries
- Demo: fuzzy string matching for vessel name returns relevant sanctions records
- Demonstrates: sanctions screening concept, Snowflake SQL queries

#### POC 4: Single-Agent LangGraph Demo
- Minimal LangGraph graph with one News Analyst agent
- Agent receives a question about Red Sea shipping risk → queries GDELT Events data via `get_chokepoint_risk` tool → produces a cited risk assessment using Pydantic output schema
- Demonstrates: agentic workflow, structured output, MCP tool call
...
| **M3** | LLM Integration + Agents + Guardrails | Apr 9–15 | LangGraph supervisor + 5 agents; MCP server with 6 tools; Redis caching layer; Pydantic schemas; HITL gate |
...
| **M7** | Testing + Evals | Apr 18–22 | Golden set evals, unit/integration tests, CI pipeline green |
...
| API cost overrun (OpenAI/Claude) | Medium | Medium | GPT-4o-mini for high-volume tasks; prompt caching; daily token budget caps |
...
|---|---|---|
| Data sources integrated | 6 + chokepoint seed | Count of operational pipelines |
| Total data processed | ≥ 16 GB | Snowflake storage + query volume |
...
- **Academic Value:** Bridges big data engineering and agentic AI—two of the most important trends for 2025–2030. Addresses the data-heavy requirement while pushing the boundary on LLM integration with novel open-source libraries (MovingPandas, Kepler.gl, PyGWalker).
...
Maritime AI Sentinel addresses one of the most consequential challenges in global trade: the inability to rapidly assess and respond to geopolitical disruptions affecting maritime supply chains. By fusing real-time AIS vessel tracking with GDELT geopolitical events, ACLED conflict data, OFAC sanctions intelligence, and weather alerts across 6 data sources totaling 16+ GB, this project creates a comprehensive maritime risk intelligence platform.

The technical architecture—Snowflake for structured analytics, Redis caching for performance, LangGraph multi-agent reasoning with MCP tool protocol, and Kepler.gl interactive visualization—demonstrates mastery of the core DAMG 7245 competencies: big data engineering, significant LLM use, cloud-native architecture, and user-facing application design. The integration of novel open-source libraries (MovingPandas for trajectory analytics, PyGWalker for interactive EDA) differentiates this project from standard approaches.
